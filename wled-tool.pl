...
#   the windows executable.  pp -o wled-tool.exe wled-tool.pl
#
# PERL VERSION:  5.28.1
# ==============================================================================
use strict;
...
getopts('hdilrye:f:v:w:x:a:A:c:C:g:G:p:P:m:M:', \%cliOpts);  # Load CLI options hash
...
GENERAL DESCRIPTION
   ...
   A confirmation prompt is presented to the user for options that will change
   either the WLED instance or an existing file. This prompt can be suppressed
...
USAGE:
   ...
   -d           Run in debug mode.
   -y           Default yes to confirmation prompts.
...
EXAMPLES:
   ...
   $ExecutableName -y -G 'palette*.json'
      Restore all palette files in current working directory to the WLED at
      IP $WledIp. The user confirmation prompt is suppressed.
   ...
   $ExecutableName -y -P myPresets.json -C myConfig.json
      Restore preset and configuration data to the WLED at IP address 4.3.2.1 
      from the specified files. WLED automatic restart activates the data. User
...
sub ValidateJson {
   ...
   if ($ArrayPointer ne '') {
      ...
      $check = join('', @array);
      return 1 if (&chkJson(\$check, $Quiet));
...
sub FormatPreset {
   ...
   if (exists($$JsonRef{'playlist'})) {
      ...
      my $playref = $$JsonRef{'playlist'};                 # playlist array reference
      my %done2 = ();
   ...
   if (-e $File) {
      ...
      if (&ValidateJson(\@data, '', '', '')) {
         ...
         if ($File =~ m/"palette":\[(.+)\]/s) {
            ...
            if ($exitCode == 1) {
               &ColorMessage("PostUrl - Invalid palette: $File", "BRIGHT_RED", '');
               return $exitCode;
            }
         }
   ...
   if ($Cmd =~ m/^b/ or $Cmd =~ m/^s/ or $Cmd =~ m/^i/) {
      ...
      # ==========
         ...
         foreach my $segref (@{ $s_ref->{'seg'} }) {    # Show user current values.
            &ColorMessage("  seg: $segref->{'id'}   fx: $segref->{'fx'}   " .
                          "sx: $segref->{'sx'}   ix: $segref->{'ix'}", "CYAN", '');
         }
         return 0;
      }
...
#
# DESCRIPTION:
#    This routine prompts the user for a Yes/No confirmation. Returns yes
#    without prompt if $DefaultY is specified or the specified backup file
#    does not exist.
#
# CALLING SYNTAX:
#    $result = &Confirm($Option, $File, $DefaultY);
#
# ARGUMENTS:
#    $Option         CLI option being processed. (A,a,C,c,P,p,G,g,M,m)
#    $File           File being processed.
#    $DefaultY       Set if -y CLI option is specified.
#
# RETURNED VALUES:
#    0 = No,  1 = Yes.
#
...
# =============================================================================
sub Confirm {
   my($Option, $File, $DefaultY) = @_;
   
   return 1 if ($DefaultY eq 1);
   
   if ($Option =~ m/^[ACPGM]$/) {
      if ($Option =~ m/^G/) {
         &ColorMessage("Restore WLED palettes? [y/N] -> ", 
                       "BRIGHT_YELLOW", 'nocr');
      }
      elsif ($Option =~ m/^M/) {
         &ColorMessage("Restore WLED ledmaps? [y/N] -> ", 
                       "BRIGHT_YELLOW", 'nocr');
      }
      else {
         &ColorMessage("Restore WLED from $File ? [y/N] -> ", 
                       "BRIGHT_YELLOW", 'nocr');
      }
   }   
   elsif ($Option =~ m/^[acpgm]$/) {
      if ($Option =~ m/^g/) {
         if (glob(join('/', $File, 'palette*'))) {
            &ColorMessage("Overwrite existing palette file(s)? [y/N] -> ", 
                          "BRIGHT_YELLOW", 'nocr');
         }
      }
      elsif ($Option =~ m/^m/) {
         if (glob(join('/', $File, 'ledmap*'))) {
            &ColorMessage("Overwrite existing ledmap file(s)? [y/N] -> ", 
                           "BRIGHT_YELLOW", 'nocr');
         }
      }
      else {
         if (-e $File) {   
            &ColorMessage("Overwrite existing $File ? [y/N] -> ", 
                          "BRIGHT_YELLOW", 'nocr');
         }
         else {
            return 1;
      ...
      &ColorMessage("Unhandled CLI option: '$Option'", "BRIGHT_RED", '');
      return 0;
   }
...
# ==========
# Set default confirmation if user specified -y.
if (exists( $cliOpts{y} )) {
   $cliOpts{y} = 1;
}
else {
...
if (exists( $cliOpts{p} )) {
   ...
   exit(0) unless (&Confirm('p', $cliOpts{p}, $cliOpts{y}));
   ...
   foreach my $pal (@pals) {
      if ($pal > 245) {
         next if (exists($reported{$pal}));   # Only report a palette id once.
         my $n = 255 - $pal;
...
if (exists( $cliOpts{c} )) {
   ...
   exit(0) unless (&Confirm('c', $cliOpts{c}, $cliOpts{y}));
...
if (exists( $cliOpts{a} )) {
   exit(0) unless (&Confirm('a', $cliOpts{a}, $cliOpts{y}));
...
if (exists( $cliOpts{A} )) {
   ...
   if (grep /== eof ==/, @data) {
      foreach my $section ('presets','palettes','ledmaps','configuration') {
         ...
         if ($beg == -1) {
            &ColorMessage("Expected section '$section' not found in $cliOpts{A}",
                          "BRIGHT_YELLOW", '');
            next;
         }
...
if (exists( $cliOpts{m} )) {
   if (-d $cliOpts{m}) {
      exit(0) unless (&Confirm('m', $cliOpts{m}, $cliOpts{y}));
...
if (exists( $cliOpts{M} )) {
   ...
   foreach my $file (@files) {
      $file =~ s/^\s+|\s+$//g;
      $file = join('.', $file, 'json') unless ($file =~ m/\.json$/); 
      if (-e $file and not -d $file) {
         exit(1) if (&PostUrl(join("/", $WledUrl, "upload"), $file));
...
if (exists( $cliOpts{g} )) {
   if (-d $cliOpts{g}) {
      exit(0) unless (&Confirm('g', $cliOpts{g}, $cliOpts{y}));
